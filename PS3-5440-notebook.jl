...
# ╔═╡ fc26d9f3-4b96-4ae8-a064-e31d2d8e7d75
#Add additional column to S (so it is M by R+1). Last col has Xa. Multiplied by column array with rates and 'u' value in last row.
#u < Vr+1 < u
#Rates are in columns of arrays
...
# ╔═╡ d666790c-e948-4226-a615-3c2726f644ab
#(mol/L)*Vhl60*(mol/cell)*(1 cell / WeightHL60)*(g/gDW)
...
# ╔═╡ 1583aa3b-0663-4891-93f5-023aa8f0f714
df

# ╔═╡ 08020762-46a8-42b2-8371-aa45e3da09fd
begin

	# E - 
	Eₒ_2 = 0.01 # units: μmol/gDW
	u_2 = ones(6)
	E_2 = Eₒ_2.*u_2

	#Sj/Kj data for metabolites
	Met_div_Km_Asp = 96.7   #for 6.3.4.5 (v1)
	Met_div_Km_ATP = 11.9   #For 6.3.4.5 (v1)
	Met_div_Km_Fum = 0.0915  #for 4.3.2.1 (v2)
	Met_div_Km_Orn = (2.81+0.0119)/2     #For 2.1.3.3 (v4)
	Met_div_Km_Arg_v3 = 0.165 #for 3.5.3.1 (v3)      
	Met_div_Km_Arg_v5 = 73.1   #for 1.14.13.39 (v5)
	Met_div_Km = 1000     #Let this represent [Met]/Km for all other metabolites for which there is not data

	#Upper flux bounds
	v1_bound = (Met_div_Km_ATP/(1+Met_div_Km_ATP))*(Met_div_Km_Asp/(1+Met_div_Km_Asp))*(Met_div_Km/(1+Met_div_Km))
	v2_bound = (Met_div_Km/(1+Met_div_Km))    #Only one metabolite substrate (Arginosuccinate), so upper bound is about Vmax
	v3_bound = (Met_div_Km_Arg_v3)/(1+Met_div_Km_Arg_v3)*(Met_div_Km/(1+Met_div_Km))
	v4_bound = (Met_div_Km_Orn)/(1+Met_div_Km_Orn)*(Met_div_Km/(1+Met_div_Km))
	Fv5_bound = (Met_div_Km_Arg_v5/(1+Met_div_Km_Arg_v5))*((Met_div_Km/(1+Met_div_Km))^3)
	Rv5_bound = (Met_div_Km/(1+Met_div_Km))^4  
	
	# setup flux bounds array -
	flux_bounds_array_2 = zeros(ℛ,2)
	flux_bounds_array_2[:,2] .= 100.0 # default value is 100 for flux units: μmol/gDW-s
	flux_bounds_array_2[1,2] = 203.0*E_2[1]*v1_bound
	flux_bounds_array_2[2,2] = 34.5*E_2[2]*v2_bound
	flux_bounds_array_2[3,2] = 249.0*E_2[3]*v3_bound
	flux_bounds_array_2[4,2] = 88.1*E_2[4]*v4_bound
	flux_bounds_array_2[5,2] = 13.7*E_2[5]*Fv5_bound
	flux_bounds_array_2[6,2] = 13.7*E_2[6]*Rv5_bound
		

	# O2 uptake -
	flux_bounds_array_2[15,1] = 0.25

	# setup species bounds array -
	species_bounds_array_2 = zeros(ℳ,2)

	# setup the objective coefficient array -
	c_vector_2 = zeros(ℛ)

	# what is the index of the urea flux?
	idx_b₄_2 = findall(x->x=="b4", rna)[1]
	c_vector_2[idx_b₄_2] = -1

	# compute the flux -
	result_case_2 = lib.flux(S,flux_bounds_array_2,species_bounds_array_2,c_vector_2);

	# show -
	nothing
end

# ╔═╡ 06bbc073-94c7-419f-b772-6fdbf2388406
v1_bound

# ╔═╡ f848b2ef-e479-42b0-8a3a-d875e4d1af6c
v3_bound

# ╔═╡ afd484cf-2f93-47bb-bd43-bd13957c9d61
v4_bound

# ╔═╡ 437a7b4d-8d9a-4db0-8769-6a1124f6217e
Fv5_bound

# ╔═╡ 68f1384b-5d15-4f92-b24a-91b17b192790
# check:
with_terminal() do
...
# ╔═╡ f183fd67-3b25-4a7d-b1ea-8af9555a0d04
let
...
begin
	...
	#CF = 0.0
	d=12.4
	V=(4/3)*3.14159*((d/2)^3)*(10^-15) #units L
	m=10^-9 #g
	f=0.70 #frac of water
	CF=V*(1/m)*(1/(1-f))*(10^6)
